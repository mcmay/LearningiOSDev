...
class PhotoStore {
    ...
    /*let persistCon: NSPersistentContainer = {
    ...
    func fetchAllPhotosFromDisk (completion: @escaping (Result<[Photo], Error>) -> Void) {
        let fetchRequest: NSFetchRequest<Photo> = Photo.fetchRequest()
        let predicate = NSPredicate(format: "(\(#keyPath(Photo.hide)) == \(false)) AND (NOT \(#keyPath(Photo.title)) ENDSWITH '_f') AND (NOT \(#keyPath(Photo.title)) BEGINSWITH 'ha_2021')")
        fetchRequest.predicate = predicate
    ...
    func fetchFavoritePhotosFromDisk (completion: @escaping (Result<[Photo], Error>) -> Void) {
        let fetchRequest: NSFetchRequest<Photo> = Photo.fetchRequest()
